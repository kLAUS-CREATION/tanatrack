...
import type { ForgotPasswordRequest, ResetPasswordRequest } from "@/types/user/password-reset";

export const authApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    register: builder.mutation<any, RegisterUser>({
      query: (body) => ({ url: "/auth/sign-up/email", method: "POST", body }),
      invalidatesTags: ["Auth"],
    }),

    signIn: builder.mutation<any, ISignIn>({
      query: (body) => ({ url: "/auth/sign-in/email", method: "POST", body }),
      invalidatesTags: ["Auth"],
    }),
    ...
    googleOAuth: builder.mutation<any, void>({
      query: () => ({
    ...
    getSession: builder.query<any, void>({
      query: () => ({ url: "/auth/get-session", method: "GET" }),
      providesTags: ["Auth"],
    }),
