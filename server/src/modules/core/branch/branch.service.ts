...
export class BranchService {
  ...
  private async validateLimit(orgId: string) {
    ...
    const limitFeature = org.subscription?.plan.planFeatures.find(
      (f) => f.feature.key === 'max_branches',
    );
    if (!limitFeature)
      throw new BadRequestException(
        'Branch limit not configured for this plan',
      );

    const allowed = parseInt(limitFeature.value);
    if (org._count.branches >= allowed) {
      throw new BadRequestException(
        `Limit reached. Your plan allows only ${allowed} branches.`,
      );
    }
  }
  ...
  async delete(orgId: string, userId: string, branchId: string) {
    ...
    // Soft delete: preserve stock levels, sales history and ledger references.
